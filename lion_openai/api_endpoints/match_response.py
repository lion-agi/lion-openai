...
def match_response(request_model: "OpenAIRequest", response: dict | list):
    ...
    if "audio" in endpoint:
        ...
        if endpoint == "audio/transcriptions":
            if len(response) == 1:
                ...
                if "OpenAITranscriptionResponseBody" not in imported_models:
                    from .audio.transcription_models import OpenAITranscriptionResponseBody
                    imported_models["OpenAITranscriptionResponseBody"] = OpenAITranscriptionResponseBody
                ...
                if "OpenAIVerboseTranscriptionResponseBody" not in imported_models:
                    from .audio.transcription_models import OpenAIVerboseTranscriptionResponseBody
                    imported_models["OpenAIVerboseTranscriptionResponseBody"] = OpenAIVerboseTranscriptionResponseBody
                return imported_models["OpenAIVerboseTranscriptionResponseBody"](**response)
        ...
        if endpoint == "audio/translations":
            if "OpenAITranslationResponseBody" not in imported_models:
                ...
                imported_models["OpenAITranslationResponseBody"] = OpenAITranslationResponseBody
    ...
    # Chat
        ...
        if isinstance(response, dict):
            if "OpenAIChatCompletionResponseBody" not in imported_models:
                from .chat_completions.response.response_body import OpenAIChatCompletionResponseBody
                imported_models["OpenAIChatCompletionResponseBody"] = OpenAIChatCompletionResponseBody
            ...
            if "OpenAIChatCompletionChunkResponseBody" not in imported_models:
                from .chat_completions.response.response_body import OpenAIChatCompletionChunkResponseBody
                imported_models["OpenAIChatCompletionChunkResponseBody"] = OpenAIChatCompletionChunkResponseBody
            result = []
            for item in response:
                result.append(imported_models["OpenAIChatCompletionChunkResponseBody"](**item))
            return result
    ...
    # Fine-tuning
        ...
        if endpoint == "fine_tuning/jobs" and method == "POST":
            if "OpenAIFineTuningJobResponseBody" not in imported_models:
                from .fine_tuning.fine_tuning_job_models import OpenAIFineTuningJobResponseBody
                imported_models["OpenAIFineTuningJobResponseBody"] = OpenAIFineTuningJobResponseBody
        ...
        if endpoint == "fine_tuning/jobs" and method == "GET":
            if "OpenAIListFineTuningJobsResponseBody" not in imported_models:
                from .fine_tuning.list_fine_tuning_jobs import OpenAIListFineTuningJobsResponseBody
                imported_models["OpenAIListFineTuningJobsResponseBody"] = OpenAIListFineTuningJobsResponseBody
        ...
        if endpoint == "fine_tuning/jobs/{fine_tuning_job_id}/events":
            if "OpenAIListFineTuningEventsResponseBody" not in imported_models:
                from .fine_tuning.list_fine_tuning_events import OpenAIListFineTuningEventsResponseBody
                imported_models["OpenAIListFineTuningEventsResponseBody"] = OpenAIListFineTuningEventsResponseBody
        ...
        if endpoint == "fine_tuning/jobs/{fine_tuning_job_id}/checkpoints":
            if "OpenAIListFineTuningCheckpointsResponseBody" not in imported_models:
                from .fine_tuning.list_fine_tuning_checkpoints import OpenAIListFineTuningCheckpointsResponseBody
                imported_models["OpenAIListFineTuningCheckpointsResponseBody"] = OpenAIListFineTuningCheckpointsResponseBody
            return imported_models["OpenAIListFineTuningCheckpointsResponseBody"](**response)
    ...
    # Batch
        ...
        if endpoint == "batches" and method == "GET":
            ...
            if "OpenAIListBatchResponseBody" not in imported_models:
                ...
                imported_models["OpenAIListBatchResponseBody"] = OpenAIListBatchResponseBody
    ...
    # Files
        ...
        if endpoint == "files" and method == "GET":
            if "OpenAIListFilesResponseBody" not in imported_models:
                ...
                imported_models["OpenAIListFilesResponseBody"] = OpenAIListFilesResponseBody
        ...
        if endpoint == "files/{file_id}" and method == "DELETE":
            if "OpenAIDeleteFileResponseBody" not in imported_models:
                ...
                imported_models["OpenAIDeleteFileResponseBody"] = OpenAIDeleteFileResponseBody
    ...
    if "uploads" in endpoint:
        if endpoint == "uploads/{upload_id}/parts":
            ...
            if "OpenAIUploadPartResponseBody" not in imported_models:
                ...
                imported_models["OpenAIUploadPartResponseBody"] = OpenAIUploadPartResponseBody
    ...
    if "models" in endpoint:
        ...
        if endpoint == "models":
            if "OpenAIListModelResponseBody" not in imported_models:
                ...
                imported_models["OpenAIListModelResponseBody"] = OpenAIListModelResponseBody
        ...
        if endpoint == "models/{model}" and method == "DELETE":
            if "OpenAIDeleteFineTunedModelResponseBody" not in imported_models:
                from .models.delete_fine_tuned_model import OpenAIDeleteFineTunedModelResponseBody
                imported_models["OpenAIDeleteFineTunedModelResponseBody"] = OpenAIDeleteFineTunedModelResponseBody
    ...
    if endpoint == "moderations":
        if "OpenAIModerationResponseBody" not in imported_models:
            ...
            imported_models["OpenAIModerationResponseBody"] = OpenAIModerationResponseBody
        return imported_models["OpenAIModerationResponseBody"](**response)

    raise ValueError("There is no standard response model for the provided request and response")